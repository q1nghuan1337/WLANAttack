...
class Gui:
    ...
    def set_init_window(self):
        self.init_window_name.title("WIFI密码破解仿真")
        ...
        self.crack = Button(labelframe, text="开始破解", command=lambda: thread_it(self.readPassWord)).grid(column=1, row=0)
        self.label = Label(labelframe, text="字典路径：").grid(column=0, row=1)
        self.path = Entry(labelframe, width=12, textvariable=self.get_value).grid(column=1, row=1)
        self.file = Button(labelframe, text="添加字典文件", command=self.add_mm_file).grid(column=2, row=1)
        ...
        self.labelinfor = LabelFrame(width=100, height=100, text="密码破解过程")
